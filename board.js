...
// 1. 白AIの合法手を全部取得（本番盤面）
let moves = getLegalMovesOnCurrentLayer("white");
if (moves.length === 0) {
...
for (let mv of moves) {
let sim = cloneBoard(board);              // 仮想盤面をコピー
simulateMove(sim, mv.x, mv.y, mv.z, "white"); // 仮想的に白を置く

let score = minimax(sim, 1, false);       // 深さ2で未来を読む
if (score > bestScore) {
bestScore = score;
bestMove = mv;
}
